...
tError cityList_insert(tCityList * cities, tCity * city, int index) {
    ...
    // Insert at list empty
    if (list_empty && (index == INT_MAX || index == 0)) {
        cities->first = (tCityNode*)malloc(sizeof(tCityNode));
        // Check that memory has been allocated
        if (cities->first == NULL) {
            return ERR_MEMORY_ERROR;
        }
        cities->first->city = (tCity*)malloc(sizeof(tCity));
        // Check that memory has been allocated
        if (cities->first->city == NULL) {
            return ERR_MEMORY_ERROR;
        }

        city_init(cities->first->city, city->name, city->last_update, city->population, city->cases, city->critical_cases, city->deaths, city->recovered, city->medical_beds);
        cities->first->next = NULL;
    }

    // Insert at last position (list isn't empty)
    if (!list_empty && index == INT_MAX) {
        // Find last element of the list
        ptr = cities->first;
        while (ptr->next != NULL) {
            ptr = ptr->next;
        }
        // Now ptr points to last element of the list
        ...
        city_init(newCity->city, city->name, city->last_update, city->population, city->cases, city->critical_cases, city->deaths, city->recovered, city->medical_beds);
        newCity->next = NULL;

        // Next pointer to the last elements points to the new city
        ptr->next = newCity;
    }

    // Insert at first position (list isn't empty)
    if (!list_empty && index == 0) {
        // Create new city
        newCity = (tCityNode*)malloc(sizeof(tCityNode));
        // Check that memory has been allocated
        if (newCity == NULL) {
        ...
        city_init(newCity->city, city->name, city->last_update, city->population, city->cases, city->critical_cases, city->deaths, city->recovered, city->medical_beds);
        // The next pointer of the new city points to the first current element.
        newCity->next = cities->first;
        // First element is the new city
    ...
    if (index > 0 && index < INT_MAX) {
        ...
        if (index > num_elements) return ERR_INVALID;
        ...
        // New city next element points to index position element
        newCity->next = ptr->next;

        // index-1 element next pointer, points to new city
        ptr->next = newCity;
    }
